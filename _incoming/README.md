...
| 18 | [prior-art-product-overview.md](./prior-art-product-overview.md) | draft | Per-product detail: what, who for, cost, licence |

## Working order

Reading order above is narrative — it follows an item through the machine. It is the wrong
order to *write* in, because it treats all eighteen areas as equally urgent. That is the
"never ships" risk named in [prior-art.md](./prior-art.md).
...
| ---------------------------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| [open-questions.md](../open-questions.md)                        | ~~Scope~~ **answered 2026-08-15**: personal first, product possible; self-hosting required     |
| [use-cases.md](./use-cases.md)                                   | **Pick one.** Declare which use case v1 serves and which four are out of scope until it works       |
| [prior-art.md](./prior-art.md)                                   | **Run the falsification test.** Sketch the chosen use case on existing tools. What they cannot express is the actual spec |
| [prior-art-product-overview.md](./prior-art-product-overview.md) | Reference for the above. Already surveyed                                                            |
| [glossary.md](../glossary.md)                                    | **Arrived versus produced item.** One type or two, versioned or mutable-with-history. A type decision that leaks into every other file |

### Phase 1 — The core loop

The part that is actually Clackworks rather than a worse n8n. Specify only as far as the chosen
use case needs.
...
| [exhaust.md](./exhaust.md)                               | The strongest unclaimed candidate. Also defines what routing's `unroutable` means      |
...
| [human-in-the-loop.md](./human-in-the-loop.md)           | Gates attach to pipeline steps, so steps come first          |
